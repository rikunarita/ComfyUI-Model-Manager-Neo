...
| `hero.webm`                  | README, docs  | the same tour as a lossless-enough recording; `hero.gif` is derived from it (`ffmpeg -vf fps=10,scale=900:-1:flags=lanczos,split[s0][s1];[s0]palettegen[p];[s1][p]paletteuse`) |
...
| `view-folders.png`           | README        | **Folder** layout one level deep, with the breadcrumb trail                                                                                                                    |
...
| `model-edit.png`             | README, docs  | the same window in **edit mode**: type dropdown, folder button, name field with its folder-prefix hint                                                                         |
...
| `download.png`               | docs          | **Create Download Task** with a direct URL typed in                                                                                                                            |
...
| `loading-panel.png`          | docs          | the **panel-scoped** loading scrim: only the manager window is dimmed/blurred                                                                                                  |
