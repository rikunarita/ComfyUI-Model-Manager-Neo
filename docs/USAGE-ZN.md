...
- **预览** — `默认`（轮播）/ `网络`（粘贴图片或视频 URL）/ `本地`（拖放或选择文件；
  图片转 WebP，视频保留原格式）/ `无`（删除全部预览文件）。来源切换仅存在于模型
  详情编辑器；下载对话框只使用从模型页解析出的图集作为唯一预览来源。编辑模式下预览下方的
  缩略图网格可管理图集（选择主预览、左右移动、删除单张）；网格末尾的虚线方块可把
  本地图片文件添加为新预览。保存时停留的那一页成为卡片的**主预览**。
...
3. Civitai / HF 页面会解析出一个或多个**版本**与**文件**：版本在工具栏选，
   文件在编辑器里选。版本行以下的编辑器把预览放在左侧、图库网格放在右侧，
