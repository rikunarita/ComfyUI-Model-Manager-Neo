...
5. [模型详情与编辑](#5-模型详情与编辑)
6. [下载模型](#6-下载模型)
7. [上传到 Hugging Face / ModelScope](#7-上传到-hugging-face--modelscope)
8. [从本地文件上传](#8-从本地文件上传)
...
重启 ComfyUI。Python 依赖（`huggingface_hub`、`hf_xet`、`modelscope_hub`、`markdownify`）会在首次
...
可同时打开多个窗口，点击某个窗口会把它带到最前。管理器窗口的标题栏会实时显示
整个库的模型总大小。
...
- **智能集合**（展平视图）— 将当前搜索+类型过滤保存为命名集合（按用户持久化），
  通过集合菜单一键重新应用；应用中的集合以芯片显示，可清除/删除。保存与应用
  收纳在同一枚胶囊按钮 `[💾 集合 ▾]` 中：软盘部分保留自己的点击/键盘目标
  （打开保存对话框），按钮的其余部分打开应用/切换已保存集合的菜单。
- **卫生扫描**（标题栏按钮，两种布局均可用）— 仅本地（不使用网络、不计算哈希）列出孤立预览/笔记、
...
- **Information 标签** — 汇总模型所记录全部信息的表格（默认只读；编辑模式下铅笔
  ...
  张量结构以**文件夹树**呈现（张量名按点号分段成层，每层文件夹行带文件夹图标，
...
- **文件名** — 支持**文件夹前缀**。输入 `subfolder/my-model` 后保存，模型会被放入
  ...
  `.`、`..` 段会被拒绝，后端还会再次做路径穿越校验。
- **预览** — 作为单一图集管理。
  编辑模式下主预览旁/下方显示**内联横向滚动的缩略图条**，可选择主预览、左右移动、
  删除单张；带**蓝色圆环**的缩略图即保存后晋升为卡片**主预览**的那一张（点击缩略图或
  ‹ / › 按钮移动圆环）。条末的虚线方块可把本地图片文件添加为新预览（图片转 WebP，
  视频保留原格式）；删除全部条目即为无预览。编辑模式下主预览以卡片宽度的 4/5 靠左显示，
  缩略图条使用其余宽度。进入编辑时圆环总是落在**当前主预览**上，与浏览模式下正停留在
  哪一页无关；浏览模式的翻页只改变显示，不会改变主预览指定。
...
3. Civitai / HF 页面会解析出一个或多个**版本**与**文件**：版本在工具栏选，
   文件在编辑器里选。版本行以下的编辑器把预览放在左侧、图库缩略图条放在右侧，
...
4. 编辑器中可设置目标类型/目录、文件名（可带文件夹前缀）、预览图与 Markdown 描述
   ...
   都会保留，轮播中选中的那张会成为卡片的主预览。下载对话框直接以该解析图集
   作为预览。
...
当输入**不以** `https://` 开头时,同一输入框变为模型名搜索框:结果并行来自
**Hugging Face**(左)、**ModelScope**(中)、**Civitai**(右)。每行包含所有者的头像
(无头像时为首字母徽章)、下载量,以及两个深度链接——所有者名→所有者页面、仓库名→
...
放大 Civitai 来源模型的预览时,左侧显示图像,右侧显示解析后的生成元数据:提示词、
负面提示词、采样器、步数、CFG 缩放、种子、Clip skip、尺寸、基础模型与资源配方。
...
## 7. 上传到 Hugging Face / ModelScope

向导先选择**上传平台**（Hugging Face / ModelScope），然后经过类型/模型选择进入上传表单。
ModelScope 始终连接国际域名 `www.modelscope.ai`。仓库创建（公开/私有）、仓库内目标路径
与实时进度对两个平台通用；文件夹视图的批量上传（所选文件夹内全部模型、保留子文件夹
结构）也遵循所选平台。

先设置令牌：**设置 → Model Manager Neo → 密钥 → Hugging Face API 密钥**
（或导出环境变量 `HF_TOKEN`）；上传到 ModelScope 时使用 **ModelScope API 密钥**
（或 `MODELSCOPE_API_TOKEN`）。

打开标题栏的**上传到 Hugging Face / ModelScope**：
...
4. **上传**：
   ...
   - **同时上传相关资源(预览/笔记)** — 将所有 `<模型名>.*` 的侧车文件(预览图与
     Markdown 笔记)上传到仓库中模型所在的目录。
   - **将模型笔记作为 README.md 上传**(默认 ON) — 模型的 Markdown 笔记将以仓库的
     `README.md` 提交,而不是 `<模型名>.md`。
   - **仓库内目标路径** — 仓库内的目录 + 文件名。
...
- **上传到 Hugging Face / ModelScope**（选中文件夹时） — 通过同一个上传对话框批量上传所选文件夹内的
  全部模型（保留子文件夹结构）;
...
并附带 Linux x86_64(CPython 3.10–3.15)的**预编译 `zipnn_core` 二进制**。在这些平台上，
...
- **Hugging Face API 密钥** / **Civitai API 密钥** / **ModelScope API 密钥** —
...
| `Hugging Face token not set`  | 在设置（或 `HF_TOKEN`）中设置令牌后重新打开对话框                                           |
