...
ZipNN 打包文件夹(`*_ZNN`)与普通文件夹不能同时选择：尝试混选时打包文件夹会被
自动取消勾选并弹出警告提示。
...
模型迁移），文件夹重命名为 **`<name>_ZNN`**。打包文件夹是密封的：内部只允许
`*.znn.*` 模型（普通模型的上传/下载/移入会被拒绝）；在 `*_ZNN` 文件夹上按下批量
按钮则**解压**并恢复原名。模型类型**根文件夹**（如 `checkpoints`）**就地**处理、
永不重命名（重命名会使其脱离 ComfyUI 的目录映射），方向自动判定：有未压缩模型则
压缩，仅剩压缩包则解压；任务进行中按钮显示圆环且**环内带百分比**。多个文件夹在
一次确认后顺序执行。
