...
5. [模型详情与编辑](#5-模型详情与编辑)
6. [下载模型](#6-下载模型)
7. [上传到 Hugging Face](#7-上传到-hugging-face)
8. [从本地文件上传](#8-从本地文件上传)
...
- **文件名** — 支持**文件夹前缀**。输入 `subfolder/my-model` 后保存，模型会被放入
  ...
  `.`、`..` 段会被拒绝，后端还会再次做路径穿越校验。
- **预览** — 作为单一图集管理（旧的 `默认` / `网络` / `本地` / `无` 来源切换已移除）。
...
4. 编辑器中可设置目标类型/目录、文件名（可带文件夹前缀）、预览图与 Markdown 描述
   ...
   都会保留，轮播中选中的那张会成为卡片的主预览。下载对话框以该解析图集为唯一
   预览来源（不提供来源切换）。
...
| **Hugging Face 已存储该内容**                           | 相同字节已存在于仓库对象存储中，因此 Hugging Face 未传输任何数据（`Upload 0 LFS files`），但**已创建指向它们的新提交**；提示中附文件链接 |
...
- **上传到 Hugging Face**（选中文件夹时） — 通过同一个上传对话框批量上传所选文件夹内的
  全部模型（保留子文件夹结构）;
