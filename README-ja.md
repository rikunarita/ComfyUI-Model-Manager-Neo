...
- [なぜ Neo なのか](#why-neo) · [UI ツアー](#screenshots) · [インストール](#installation) ·
  [機能](#features)
- [モデル検索とマルチプラットフォーム発見](#search) · [ZipNN 無劣化圧縮](#zipnn) ·
  [オリジナルからの変更点](#what-changed) ·
...
- タイプルートのフォルダーカードは、その**タイプの合計サイズ**を表示します。また、
  ...
  赤い**重複警告**を出します。
- タイプルートの配下に保存されたモデルは、カードの名前の上に**サブディレクトリ**を
  表示します(フラット/フォルダー両ビュー)。
- **スマートコレクション** — フラットビューの現在の検索 + タイプフィルタを、名前付きの
  コレクションとしてユーザー単位で保存し、ワンクリックで再適用できます。**保存**
  (フロッピー)と**コレクション**の操作は 1 つのボタンに統合されています: フロッピー
  アイコンはラベルの前、わずかな余白をあけてコレクションボタン**内**に配置され、
  かつての隣同士が `[💾 コレクション ▾]` の 1 つのピルとして読めます。フロッピーは
  独自のクリック/キーボードターゲット(保存ダイアログを開く)を保ち、ボタンの残りは
  保存済みコレクションの適用/切り替えメニューを開きます。
...
- **Civitai**・**Hugging Face**・**ModelScope**(`www.modelscope.ai`)・**直接ファイルリンク**
  の URL を貼り付けて利用できます。
- Civitai のミラーホスト `civitai.red` のページ URL も**Civitai** と同じく扱われます
  (解決後に保存されるモデルページも入力されたホストを保ちます)。
...
- **Hugging Face / ModelScope へ**: ウィザードの最初のステップでプラットフォームを
  選択(各ハブはロゴ入りタイルで表示)。対応するトークンで認証し、リポジトリが存在しなければ作成(公開/非公開を
...
- モデルのリネーム、フォルダー/タイプ間の移動、プレビューやノートを含めた**完全削除**。
  未保存の変更がある状態でキャンセルすると、確認ダイアログが先に出ます。
- モデルの隣に保存された Markdown ノートの閲覧・編集・保存。Information テーブル自体も、
  警告付きで編集できます(保存時にノートのフロントマターを書き換えます)。
- プレビュー画像の変更・削除 — 編集モードでは**青枠(リング)が付いたサムネイル**が、
  保存時にカードのプライマリプレビューになる画像です。タイルのクリックや ‹ / ›
  ボタンでリングが移動し、編集画面を開いた時点のリングは常に**現在のプライマリ画像**
  に付きます(閲覧モードで何枚目を見ていたかには影響されず、閲覧中のページ送りが
  プライマリ指定を変えることもありません)。編集モードのギャラリーストリップ末尾の
  点線タイルから、ローカル画像を追加できます。
- **モデルページを開く**アクションのボタン背景には、モデルの出所(Civitai /
  Hugging Face)のロゴが表示されます。
- **ローカルにダウンロード**アクションは、保存済みファイルを添付ファイルとして
  ブラウザへ配信し、ライブラリどおりの名前で保存します。詳細アクション行全体
  (ZipNN・スター・ハブページ・識別・グラフ操作・ローカルダウンロード・編集・削除)は
  1 つのインラインスクロール行内で同一ボタンサイズに統一されています。
...
## <img src="https://api.iconify.design/lucide/search.svg?color=%2314b8a6" width="28" height="28" align="middle" alt=""> モデル検索とマルチプラットフォーム発見

**ダウンロードタスクを作成** ウィンドウはページ URL 以外も受け付けます。
`https://` で**始まらない**入力はすべてモデル名クエリとして扱われ、3 プラットフォームを
並列検索します — **Hugging Face**(左列、`huggingface_hub` の `HfApi.list_models` 経由)、
**ModelScope**(中央、`modelscope_hub` 経由)、**Civitai**(右、公開 REST API)。
`https://` のプレフィックスは入力途中から 1 文字ずつ判定されるため、フィールドは
検索モードと URL モードの間をリアルタイムに切り替わります。結果は短いデバウンスで
更新され、各列は検索全体を失敗させることなく個別にエラーを報告します。

- 各結果行は、公開ユーザー/組織の**アバター**を角丸枠で表示します。
  Hugging Face は組織・個人とも API(`/api/organizations/{name}` と
  `/api/users/{name}`)で解決し、ModelScope は
  `GET https://www.modelscope.ai/api/v1/models/{owner}/{repo}` のオーナー
  (`Organization`)ブロックからアバターと表示名を解決します。
  ハブ側がアバターを持たない場合はイニシャルバッジになります。加えて通算ダウンロード数も表示します。
- モデル ID は 2 つのディープリンクに分割されます。**オーナー名**はユーザー/組織ページへ、
  **リポジトリ名**はモデルページへ — いずれもホバーで下線が表示され、タップで
  ブラウザーに開きます。行のそれ以外をクリックすると、そのモデルがダウンロード
  エディターへそのまま解決されます。
- スキーム無しの **`username/repo-name`** 入力にも対応し、**Enter 一発で必ず解決**
  します: 結果内の完全一致 → 裸のリポジトリ ID を Hugging Face リポジトリへ →
  最初の非空列の先頭候補、の順。結果がまだ無い場合は即座に名前検索を実行します。
- 各列はページ送り対応: 列の一番下までスクロールすると次ページが存在する限り
  **「∨ さらに表示」** が現れ、押すとその列の次ページを追加取得します(Hugging Face
  はダウンロード順オフセット、ModelScope はページ番号、Civitai は API 自身のカーソル)。
- プラットフォームは **設定 → Model Manager Neo → 検索** でユーザーごとに
  非表示にできます(プラットフォーム毎の真偽値)。
- 各プラットフォームの**並び順**も同じ設定項目で選択できます(各 API が受け付ける
  すべての値)。既定は Hugging Face がトレンド順、ModelScope が Like 多い順、
  Civitai が高評価順です。

Civitai ダウンロードには、公式 CLI が普及させた安全網をマネージャーのタスク機構に
合わせて適用しています:

- **ダウンロード計画(ドライラン)** — 開始前に、解決済みの保存先パス、公表サイズ、
  公開 SHA256、プラットフォーム API キーの設定状況を表示します。
- **SHA256 突合** — 完了した Civitai ダウンロードはハッシュ化され、公開 SHA256 と
  突合されます。不一致の場合はファイルを削除し、タスクを失敗させます。
- **レイアウトルーティング** — 自身の種別が別のモデルフォルダーに対応する版ファイル
  (同梱 VAE など)は、選択中のフォルダーではなく該当フォルダーへ格納されます。
- **ベースモデル警告** — 版のベースモデルが、保存先フォルダーのライブラリに記録された
  ベースモデル群と異なる場合に警告します。
- **実行可能形式警告** — pickle / アーカイブ系ペイロードは読込時にコードを実行し得る
  ため、ダウンロード前にその旨を表示します。
- **ハブアカウント(whoami)** — キー設定済みの各プラットフォーム(Hugging Face・
  ModelScope・Civitai)について、接続中アカウントをダウンロードダイアログに
  それぞれの whoami エンドポイント経由で表示します。401 失敗時にはキーの作成場所と
  再開方法を正確に案内します。

さらに、Civitai 由来モデルのプレビューを拡大すると、ライトボックスに画像を左、
解析済みの**生成メタデータ**を右へ表示します(プロンプト、ネガティブプロンプト、
サンプラー、ステップ数、CFG スケール、シード、Clip skip、サイズ、ベースモデル、
使用リソース)。メタデータは画像ごとに Civitai API から取得します。

加えて、モデル詳細ウィンドウの**ハッシュで識別**は、Civitai カタログへ当該ローカル
ファイルがどのモデルバージョンであるかを逆引きします(ハッシュによる逆引きルックアップ):
Markdown サイドカーに記録済みのハッシュを先に試し、いずれも命中しないときだけ
ファイルのハッシュを 1 パスで計算します(`SHA256` / `AutoV2` / `AutoV1` /
`CRC32`、利用可能な場合は `BLAKE3` も)。命中時は解決されたモデル/バージョンと
ベースモデル・トリガーワード・ファイル一覧、そして公式 CLI が命中時に出力するのと
同じ `civitai download` コマンドを表示します。不一致の場合は、一致するモデル
バージョンが無い旨を伝えます。
...
> プレビュー/ノートを手動で設定する(モデルエディターのギャラリーストリップ:
> 点線タイルからローカル画像を追加、またはサムネイルの並べ替え/削除)か、
> プレビュー付きで再ダウンロードする(_Create Download Task_
