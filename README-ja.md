...
- **HuggingFace へのアップロード**(ステップ 3): リポジトリ ID、作成時のプライベート
  指定、アップロード先パス。
...
- **Hugging Face / ModelScope へ**: フォームでプロバイダーを選択。対応するトークンで
  認証し、リポジトリが存在しなければ作成(公開/非公開を選択)、アップロード先パスを
  指定して進捗を確認できます。ModelScope は常に国際ドメイン `www.modelscope.ai` へ
  接続し、そのロゴは他のハブと同じく「モデルページを開く」ボタンの背景になります。
...
Civitai / HuggingFace のページパーサー、カードをグラフへドラッグする統合、モデル
