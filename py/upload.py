...
class ModelUploader:
    def add_routes(self, routes):
        ...
        async def upload_model(request):
            ...
            try:
                ...
                utils.print_info("Upload model success")
    ...
    def validate_upload_target(self, file_folder: str, filename: str):
        """
        SECURITY FIX (inherited from upstream): the multipart upload used to
        write to whatever "folder"/"filename" the client sent, allowing
        arbitrary file writes and path traversal for anyone able to reach the
        ComfyUI server. The legitimate UI only ever offers directories inside
        the known model folders, so validating the target does not change any
        intended functionality.
        """
        if not file_folder or not filename:
            raise RuntimeError("Invalid upload target: missing folder or filename")

        if filename in (".", "..") or "/" in filename or "\\" in filename or "\x00" in filename:
            raise RuntimeError(f"Invalid file name: {filename}")

        if self.resolve_model_folder(file_folder) is None:
            raise RuntimeError(f"Target folder is not inside a known model folder: {file_folder}")

        base = utils.normalize_path(file_folder)
        target = utils.join_path(base, filename)
