...
- <img src="https://api.iconify.design/lucide/package-plus.svg?color=%23f59e0b" width="16" height="16" align="middle" alt=""> **ZipNN 可逆圧縮** — safetensors モデルをその場で圧縮/解凍
  ...
  フォルダ単位では封印された `<name>_DeltaZNN` バンドルへバッチ圧縮され（圧縮済み
  モデルはすべてバンドルへ移動）、ファインチューンモデルはベースに対する極小の
  **デルタファイル**に縮みます — _Neo の新機能_。
...
追従）、**バンドルフォルダ `<name>_DeltaZNN` へ移動**します — 空になった元の
フォルダは消えます。`*_DeltaZNN` バンドルは封印されています:

- 内部に置けるのは ZipNN の内容物（`*.znn.*` モデルと `*.znn` デルタファイル）のみ
  （プレーンモデルのアップロード・ダウンロード・移動は拒否されます）。
- バンドルフォルダと非バンドルフォルダの同時選択は不可 — バンドル側が警告トーストと
  ともに自動で選択解除されます。
- バンドルの ZipNN ボタンは**反転表示**になり、押すと**バッチ解凍** — 中身は元の
  フォルダへ戻り、空になったバンドルフォルダは削除されます。
- デルタフォルダ（`<base>_DeltaZNN`、後述）もバンドルです: 反転ボタンを押すと内部の
  ファインチューンモデルがまとめて復元されます。
- モデル種別の**ルートフォルダ**（`checkpoints` など）はバンドルを**自分の中に**
  作ります（`<root>_DeltaZNN`）— ルートの外（兄弟ディレクトリ）に作ると ComfyUI の
  フォルダ対応から外れ、ローダーからもマネージャからも見えなくなるためです。方向は自動判定
  で、未圧縮モデルがあれば圧縮、圧縮済みのみなら解凍です。
- 旧バージョンが作った `<name>_ZNN` バンドルも引き続き認識され、元の名前へ解凍
  されます。
- 進捗中はボタンが円形リングになり、**円内にパーセント表示**が出ます。
