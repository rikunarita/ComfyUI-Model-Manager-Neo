<template>
  <div
    class="mm-transition cursor-pointer rounded-mm-card border-2 border-dashed border-mm-border p-4 text-mm-muted-fg"
    :class="{
  ...
  >
    <slot name="default">
      <div class="flex h-full flex-col items-center justify-center gap-2">
        ...
        <p class="m-0 overflow-hidden text-sm text-ellipsis select-none">
          {{ $t('uploadFile') }}
        </p>
      </div>
    </slot>
...
import { ref } from 'vue'
import { type SelectEvent, type SelectFile } from 'types/typings'
